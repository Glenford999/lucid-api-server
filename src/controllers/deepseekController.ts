...
export const productSearch = async (req: Request, res: Response) => {
  try {
    ...
    console.log('DeepSeek API configuration status:');
    console.log(`API Endpoint configured: ${!!deepseekApiEndpoint} (${deepseekApiEndpoint})`);
    console.log(`API Key configured: ${!!deepseekApiKey} (length: ${deepseekApiKey ? deepseekApiKey.length : 0})`);
    console.log(`API Key masked: ${deepseekApiKey ? deepseekApiKey.substring(0, 3) + '...' + deepseekApiKey.substring(deepseekApiKey.length - 3) : 'None'}`);
    ...
    if (!deepseekApiKey || !deepseekApiEndpoint) {
      ...
      return res.status(200).json({
        ...
        is_mock: true,
        reason: !deepseekApiKey ? 'API_KEY_MISSING' : 'API_ENDPOINT_MISSING'
      });
    }
