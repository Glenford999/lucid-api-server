...
- Validate JSON syntax`;
    ...
    logger.info(`[${requestId}] Making request to DeepSeek API at ${deepseekApiEndpoint} for query: "${query}"`);
    ...
    try {
      logger.info(`[${requestId}] Sending API request to DeepSeek...`);
      ...
      for (const endpoint of apiEndpoints) {
        try {
          logger.info(`[${requestId}] Trying API endpoint: ${endpoint}`);
          ...
          logger.info(`[${requestId}] API request successful on endpoint ${endpoint}`);
          break;  // If successful, exit the loop
        } catch (error: any) {
          lastError = error;
          logger.warn(`[${requestId}] API request failed on endpoint ${endpoint}: ${error.message}`);
      ...
      logger.info(`[${requestId}] API request completed in ${apiRequestDuration}ms with status ${deepseekResponse.status}`);
      
      // Log response structure without full content
      logger.debug(`[${requestId}] Response structure:`, {
      ...
      logger.info(`[${requestId}] Processing response data...`);
      const processedResponse = processDeepSeekSearchResponse(deepseekResponse.data);
      
      logger.info(`[${requestId}] Processed response structure:`, {
      ...
      logger.error(`[${requestId}] DeepSeek API request failed:`, errorDetails);
      logger.info(`[${requestId}] ===== END: Product Search Request (ERROR) ===== Duration: ${Date.now() - startTime}ms`);
      
      return res.status(502).json({
        success: false,
        error: 'Failed to retrieve search results. The API service may be unavailable.',
        details: errorDetails,
        request_id: requestId
      });
    }
  } catch (error: any) {
    logger.error(`[${requestId}] Unexpected error in product search controller:`, error);
    logger.info(`[${requestId}] ===== END: Product Search Request (ERROR) ===== Duration: ${Date.now() - startTime}ms`);
...
export const productComparison = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!product_ids || !Array.isArray(product_ids) || product_ids.length < 2) {
      console.log(`[${requestId}] Invalid request: at least two product_ids are required`);
      console.log(`[${requestId}] ===== END: Product Comparison Request (BAD REQUEST) ===== Duration: ${Date.now() - startTime}ms`);
      return res.status(400).json({
        success: false,
    ...
    console.log(`[${requestId}] Comparison request received for products: ${product_ids.join(', ')}`);
    console.log(`[${requestId}] Options: comparison_type=${comparison_type || 'default'}, include_categories=${!!include_categories}, include_conclusion=${!!include_conclusion}`);
    ...
    if (!deepseekApiKey || !deepseekApiEndpoint) {
      console.warn(`[${requestId}] DeepSeek API not configured, returning error`);
      const reason = !deepseekApiKey ? 'API_KEY_MISSING' : 'API_ENDPOINT_MISSING';
      console.log(`[${requestId}] Error reason: ${reason}`);
    ...
    try {
      ...
      console.log(`[${requestId}] ===== END: Product Comparison Request (API ERROR) ===== Duration: ${Date.now() - startTime}ms`);
