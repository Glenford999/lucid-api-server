    ...
    const normalizedResults = results.map((result) => {
      ...
      if (hasNewFormat) {
        ...
        const purchasingOptions = Array.isArray(result.purchasing_options) 
          ? result.purchasing_options.map((option) => ({
        ...
        return {
          ...
          retailers: Array.isArray(result.retailers) ? result.retailers.map((retailer) => ({
...
function generateMockSearchResponse(query, filters = {}) {
  const requestId = `mock_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;
