...
export async function testDeepSeekApiConnection(
  ...
  try {
    ...
    return {
      ...
      details: { testedEndpoints: healthEndpoints },
    };
  } catch (error: any) {
...
export async function makeDeepSeekRequest(
  ...
  logger.debug(`[${requestId}] Request payload query:`, payload.query);
  ...
  // Normalize endpoint to ensure proper formatting
  ...
  try {
    ...
    logger.info(`[${requestId}] Full DeepSeek API URL: ${fullUrl}`);
    logger.debug(`[${requestId}] Request headers:`, JSON.stringify(headers, null, 2).replace(sanitizedApiKey, '***'));
    ...
    // Try common alternative paths if we got a 404
    if (error.response && error.response.status === 404) {
      logger.info(`[${requestId}] Got 404, trying alternative endpoint paths...`);
      
      // Standard endpoints to try when you get a 404
      const alternateEndpoints = [
        '/v1/chat/completions',         // This is a commonly used LLM endpoint
        '/chat/completions',            // Simplified chat endpoint
        '/completions',                 // Generic completions endpoint
        '/v1/completions',              // Versioned generic endpoint
        '/api/v1/search',               // Simplified search API
        '/search',                      // Most basic search endpoint
        '/v1/search'                    // Versioned search endpoint
      ];
      
      // Try each alternate path
      for (const altPath of alternateEndpoints) {
        try {
          const altUrl = `${baseUrl}${altPath}`;
          logger.info(`[${requestId}] Trying alternative path: ${altUrl}`);
          ...
          logger.info(`[${requestId}] Alternative path succeeded with status: ${altResponse.status}`);
          return altResponse;
        } catch (altError: any) {
          logger.info(`[${requestId}] Alternative path failed: ${altError.message}`);
