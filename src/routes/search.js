...
const checkApiKeysMiddleware = (req, res, next) => {
  ...
  if (!apiKey) {
    ...
    if (!config.isProduction) {
      ...
      return res.json(createSyntheticSearchResponse(req.body?.query || req.query?.query || 'unknown query'));
    }
...
router.post('/', checkApiKeysMiddleware, async (req, res) => {
  try {
    ...
    if (!config.isProduction) {
      console.log('Using synthetic data in development mode');
      const response = createSyntheticSearchResponse(query);
      return res.json(response);
    } else {
      // Make actual API calls to DeepSeek 
      try {
        ...
        response.error_message = apiError.message;
        return res.json(response);
      }
    }
  } catch (error) {
...
router.get('/', checkApiKeysMiddleware, async (req, res) => {
  try {
    ...
    if (!config.isProduction) {
      console.log('Using synthetic data in development mode (GET request)');
      const response = createSyntheticSearchResponse(query);
      return res.json(response);
    } else {
      // Make actual API calls to DeepSeek 
      try {
        ...
        // Fall back to synthetic data if API call fails
        console.log('Falling back to synthetic data due to API error (GET)');
        const response = createSyntheticSearchResponse(query);
        response.is_fallback = true;
        response.error_message = apiError.message;
        return res.json(response);
