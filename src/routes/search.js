...
const ENDPOINTS_TO_TRY = [
  // Standard chat/completion endpoints that most LLM APIs use
  'v1/chat/completions',
  'chat/completions',
  'completions',
  'v1/completions',
  
  // Product-specific search endpoints
  'v1/product/search',
...
async function tryMultipleEndpoints(requestId, query, payload) {
  ...
  const primaryBaseUrl = config.deepseekApiEndpoint.endsWith('/') 
    ? config.deepseekApiEndpoint.slice(0, -1) 
    : config.deepseekApiEndpoint;
    
  logger.info(`[${requestId}] Trying primary API base URL: ${primaryBaseUrl}`);
  
  // First try the actual unmodified base URL with LLM completion endpoints
  // Many API services prefer to use their base URL with /v1/chat/completions
  for (const endpointPath of ENDPOINTS_TO_TRY) {
    ...
    logger.info(`[${requestId}] Trying endpoint path: ${normalizedPath}`);
    lastEndpoint = `${primaryBaseUrl}${normalizedPath}`;
    
    try {
      // Log exactly what we're trying
      logger.info(`[${requestId}] Full API URL being tested: ${lastEndpoint}`);
      
      // Call the DeepSeek API
      const apiResponse = await apiRequest.makeDeepSeekRequest(
        normalizedPath, 
      ...
      if (apiResponse.success && apiResponse.data) {
        logger.info(`[${requestId}] DeepSeek API request successful using endpoint: ${normalizedPath}`);
        logger.debug(`[${requestId}] Response data type: ${typeof apiResponse.data}`);
        
        // Log the response structure to help debugging
        if (apiResponse.data) {
          const keys = Object.keys(apiResponse.data);
          logger.debug(`[${requestId}] Response data keys: ${keys.join(', ')}`);
        }
        
        successfulResponse = apiResponse;
        break;
      } else {
        logger.error(`[${requestId}] DeepSeek API request failed for endpoint ${normalizedPath}:`, 
          apiResponse.error || 'No error details available');
        lastError = apiResponse.error || 'Unknown error';
      }
    } catch (endpointError) {
      logger.error(`[${requestId}] Error trying endpoint ${normalizedPath}:`, endpointError.message);
  ...
  if (!successfulResponse && alternativeEndpoints.length > 0) {
    ...
    for (const altApiEndpoint of alternativeEndpoints) {
      ...
      const normalizedAltBase = altApiEndpoint.endsWith('/') 
        ? altApiEndpoint.slice(0, -1) 
        : altApiEndpoint;
        
      logger.info(`[${requestId}] Trying alternative API base URL: ${normalizedAltBase}`);
      ...
      for (const endpointPath of ENDPOINTS_TO_TRY) {
        ...
        try {
          ...
          if (apiResponse.success && apiResponse.data) {
            logger.info(`[${requestId}] DeepSeek API request successful using alternative endpoint: ${lastEndpoint}`);
            successfulResponse = apiResponse;
            
            // Update the primary endpoint to use the working alternative for future requests
            config.deepseekApiEndpoint = normalizedAltBase;
            logger.info(`[${requestId}] Updated primary endpoint to: ${normalizedAltBase}`);
            break;
          }
        } catch (altError) {
          logger.error(`[${requestId}] Error trying alternative endpoint ${lastEndpoint}:`, altError.message);
  ...
  try {
    ...
    try {
      ...
      if (result.response) {
        ...
        // Attempt to provide more helpful diagnostics
        logger.error(`[${requestId}] All API endpoints failed after ${result.tryCount} attempts.`);
        
        // Try a direct check on the base API to see if the server is responding at all
        try {
          const baseUrl = config.deepseekApiEndpoint.endsWith('/') ? 
            config.deepseekApiEndpoint.slice(0, -1) : 
            config.deepseekApiEndpoint;
            
          logger.info(`[${requestId}] Testing base API URL: ${baseUrl}`);
          const baseTest = await axios.get(baseUrl, { 
            timeout: 5000,
            validateStatus: () => true // Accept any status code
          });
          
          logger.info(`[${requestId}] Base API test response: ${baseTest.status}`);
          
          if (baseTest.status < 500) {
            // Server is responding but likely the endpoint is wrong
            return sendErrorResponse(res, 404, 'The API search endpoint could not be found. The service may have changed.', {
              base_url_status: baseTest.status,
              last_attempt: result.lastEndpoint,
              tried_count: result.tryCount,
              diagnostic_url: '/api/search/deep-diagnostics',
              remediation: "The API base URL is reachable, but we couldn't find the correct search endpoint. Please check API documentation."
            });
          } else {
            // Server is having issues
            return sendErrorResponse(res, 503, 'The search service is currently unavailable', {
              base_url_status: baseTest.status,
              tried_count: result.tryCount,
              diagnostic_url: '/api/search/deep-diagnostics',
              retry_suggestion: "Please try again later. The service may be experiencing temporary issues."
            });
          }
        } catch (baseTestError) {
          // Can't even reach the API server
          return sendErrorResponse(res, 503, 'The search service is currently unavailable', {
            error: `${result.error}. Base API not reachable: ${baseTestError.message}`,
            possible_causes: [
              "API key may be invalid",
              "DeepSeek API service may be down",
              "Network connectivity issues to DeepSeek API"
            ],
            diagnostic_url: '/api/search/diagnostic'
          });
        }
      }
    } catch (apiError) {
      ...
      return sendErrorResponse(res, 502, 'Failed to get search results from API service', {
        ...
        suggestions: [
          "Check API key and endpoint in your configuration",
          "Verify the API service is operational",
          "Check the '/api/search/diagnostic' endpoint for more details"
        ]
      });
    }
  } catch (error) {
...
router.get('/diagnostic', async (req, res) => {
  try {
    ...
    const apiEndpoint = config.deepseekApiEndpoint || 'https://api.deepseek.ai';
...
router.get('/deep-diagnostics', async (req, res) => {
  try {
    ...
    if (config.deepseekApiKey) {
      ...
      for (const endpoint of ENDPOINTS_TO_TRY) {
        try {
          ...
          if (endpointTest.success) {
            ...
            // Update the config with this working endpoint for future requests
            // and break the loop since we found a working endpoint
      ...
      // Test basic API connectivity without specific paths
      try {
        // Test just the base API with a GET request
        const baseTestResponse = await axios.get(baseUrl, {
          headers: {
        ...
        diagnostics.tests.base_api_test = {
          status: baseTestResponse.status,
          headers: baseTestResponse.headers,
          data: baseTestResponse.data ? 'Data received' : 'No data',
          raw_response: baseTestResponse.data ? JSON.stringify(baseTestResponse.data).substring(0, 200) + '...' : null
        };
      } catch (baseApiError) {
        diagnostics.tests.base_api_test = {
          success: false,
      ...
      // If we didn't find any working endpoint, try alternative base urls
      if (!diagnostics.tests.direct_api_call && config.deepseekApiAlternatives) {
        // Test alternative base URLs with the most likely endpoint
        for (const altBaseUrl of config.deepseekApiAlternatives) {
          if (altBaseUrl === baseUrl) continue;
          
          const normalizedAltUrl = altBaseUrl.endsWith('/') ? 
            altBaseUrl.slice(0, -1) : altBaseUrl;
          
          try {
            // Try with the first endpoint only for alternatives
            const altTest = await testApiEndpoint(
              normalizedAltUrl,
              '/v1/chat/completions', // Most common LLM endpoint
              config.deepseekApiKey
            );
            
            if (altTest.success) {
              diagnostics.tests.alternative_base_url_test = {
                success: true,
                working_base_url: normalizedAltUrl,
                working_endpoint: '/v1/chat/completions'
              };
              break;
            }
          } catch (altError) {
            // Just log the error
            logger.error(`Alternative base URL test failed: ${altError.message}`);
          }
        }
      }
    }
    ...
    diagnostics.remediation_suggestions = [
      "Check if the API key is valid and has sufficient permissions",
      "Verify the base URL matches the DeepSeek service you're using (should be https://api.deepseek.ai)",
      "Try using the '/v1/chat/completions' endpoint which is standard for most LLM APIs",
      "Check DeepSeek documentation for the latest API endpoints",
      "Ensure your network/firewall allows outbound connections to the DeepSeek API endpoints"
    ];
