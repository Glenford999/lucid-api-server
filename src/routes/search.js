...
router.post('/', checkApiKeysMiddleware, async (req, res) => {
  try {
    ...
    try {
      console.log(`[${requestId}] Making DeepSeek API request for query:`, query);
...
- Validate JSON syntax`;
      ...
      const payload = {
        ...
        temperature: 0.3,
        max_tokens: 4000,
        context: req.body.context || { search_type: 'product' },
        filters: req.body.filters || {},
        preferences: req.body.preferences || { include_pros_cons: true }
      };
      ...
      for (const endpoint of endpointsToTry) {
        ...
        console.log(`[${requestId}] Trying endpoint: ${endpoint}`);
        ...
        try {
          ...
          if (apiResponse.success && apiResponse.data) {
            console.log(`[${requestId}] DeepSeek API request successful using endpoint: ${endpoint}`);
            successfulResponse = apiResponse;
            break;
          } else {
            console.error(`[${requestId}] DeepSeek API request failed for endpoint ${endpoint}:`, apiResponse.error);
          }
        } catch (endpointError) {
          console.error(`[${requestId}] Error trying endpoint ${endpoint}:`, endpointError.message);
        }
      }
      ...
      console.error(`[${requestId}] Error calling DeepSeek API:`, apiError.message);
...
router.get('/', checkApiKeysMiddleware, async (req, res) => {
  try {
    ...
    // Make API calls to DeepSeek 
    try {
      console.log(`[${requestId}] Making DeepSeek API request for query (GET):`, query);
...
- Validate JSON syntax`;
      ...
      const payload = {
        ...
        filters: {},
        preferences: { include_pros_cons: true }
      };
      ...
      console.error(`[${requestId}] Error calling DeepSeek API (GET):`, apiError.message);
