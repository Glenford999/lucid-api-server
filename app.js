...
logger.info(`Starting app.js in ${process.env.NODE_ENV || 'development'} mode`);
logger.info(`Current directory: ${__dirname}`);
logger.info(`Google Cloud Project: ${process.env.GOOGLE_CLOUD_PROJECT || 'not set'}`);
...
try {
  ...
  logger.info('Loaded config from ./src/config/config');
} catch (configError) {
  logger.error('Failed to load config directly:', configError.message);
  try {
    ...
    if (configPath) {
      logger.info(`Found config at ${configPath}`);
    ...
    logger.error('Failed to load config from fallback locations:', fallbackError.message);
    process.exit(1);
  }
}
...
try {
  ...
  logger.info('Loaded deepseekController');
} catch (err) {
  logger.error('Error loading deepseekController:', err.message);
  deepseekController = {
...
try {
  ...
  logger.info('Loaded search router');
} catch (err) {
  logger.error('Error loading search router:', err.message);
...
// Import API routes for testing and diagnostics
let apiRouter;
try {
  // Try TypeScript version first
  apiRouter = require('./src/routes/api').default;
  logger.info('Loaded API routes from TypeScript module');
} catch (err) {
  logger.warn('Error loading API routes from TypeScript:', err.message);
  try {
    // Fallback to JavaScript version
    apiRouter = require('./src/routes/api');
    logger.info('Loaded API routes from JavaScript module');
  } catch (fallbackErr) {
    logger.error('Failed to load API routes:', fallbackErr.message);
    
    // Create a minimal router as a fallback
    const router = express.Router();
    router.get('/test/deepseek', (req, res) => {
      return res.status(503).json({ 
        success: false,
        message: 'API route module could not be loaded'
      });
    });
    apiRouter = router;
  }
...
if (deepseekController.productComparison) {
  ...
  comparisonRouter.post('/', (req, res) => {
    ...
    logger.info(`Processing comparison for products:`, product_ids);
    
    // Return error if no API key is available
    if (!config.deepseekApiKey) {
      logger.warn('No API key available, returning error');
      return res.status(503).json({
        success: false,
  ...
  logger.info('Registered comparison route');
}
